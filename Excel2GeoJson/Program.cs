...
public class Program {
    public static void Main() {
        ...
        string outputJsonPath = "data.json";
    ...
    private static List<GeoPoint> ReadPointsFromExcel(string filePath) {
        ...
        using (var package = new ExcelPackage(new FileInfo(filePath))) {
            ...
            for (int row = 2; row <= rowCount; row++) {
                double latitude = Convert.ToDouble(worksheet.Cells[row, 1].Value);
                double longitude = Convert.ToDouble(worksheet.Cells[row, 2].Value);
                double altitude = Convert.ToDouble(worksheet.Cells[row, 3].Value);
                double timestamp = Convert.ToDouble(worksheet.Cells[row, 4].Value);
